...
/**
 * Event used for agent suspension.
 * The agents that receive this event must suspend.
 * 
 * @author G. Fortino, F. Rango
 */
public class ELDAEventSuspendRequest extends ELDAEventLifecycle {

	/**
	 * Constructor with all parameters.
	 * @param source Sender of the event
	 * @param agentsToSuspend Agents to suspend
	 */
	public ELDAEventSuspendRequest(AID source, java.util.List<AID> agentsToSuspend){
	 	super(source, agentsToSuspend); //target = agents that must suspend
	 	
	 	setLanguage(ELDAEvent.OUT);
	}
	
	/**
	 * Constructor that allows to suspend the sender of the event.
	 * @param source Sender of the event
	 */
	public ELDAEventSuspendRequest(AID source){
