 ...
 * 
 */
public class DSCStateDeleteCommand extends Command {
/** Shape to remove. */
 ...
 */
public DSCStateDeleteCommand(Container child) {
	if ( child == null) {
...
{
	//impedisco la cancellazione di uno stato se esso è  il target di una transition uscente 
	//da uno start point
	
	targetConnections = new HashMap();
	this.size2=child.getTargetRelationships().size();
	for(int i=0;i<size2;i++) {
		if (child.getTargetRelationships().get(i) instanceof Transition &&
			((Transition) child.getTargetRelationships().get(i)).getSource() instanceof StartPoint)
			return false;
	}
	return true;
}
...
public void execute() {
	...
	for(int i=0;i<size1;i++) {
		com =new ElementDeleteCommandDSC((VisualElement)child.getVisualElements().get(0));	// store a copy of incoming & outgoing connections before proceeding 
		com.execute();
		commands.add(com);
	}
}
